...
public class PessoaController : ControllerBase
{
    ...
    public Pessoa Post(Pessoa usuario)
    {
        ...
        if (n != null)
        {
            ...
            {
                string texto = x;
                var textoInvertido = new string(texto?.Reverse().ToArray());
                var letraMaiuscula = new string(textoInvertido.ToUpper().ToArray());
                var y = string.Concat(letraMaiuscula.Select((s) => " " + s.ToString()));
                usuario.contrario = y;
            }
